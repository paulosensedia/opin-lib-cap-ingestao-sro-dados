...
from opin_lib_chassis_dados.movielens.streaming.ingestion.genome_tags_schema_ingestion_schema import GenomeTagsSchemaIngestion
...
genome_tags_topic = kafka_connection \
    .option("subscribe", "MOVIELENS_GENOME_TAGS") \
...
schema = GenomeTagsSchemaIngestion.schema()
df_genome_tags = DataFrameFunctions.parse_json(genome_tags_topic, "value", "json_data", schema).select("json_data.*")


# COMMAND ----------
# DBTITLE 1,Write to bronze layer
DeltaLakeFunctions.write_delta_stream(df_genome_tags, EnumFormatTypes.DELTA.value,
    context.STORAGE_BRONZE_GENOME_TAGS_STREAM,
    context.STORAGE_BRONZE_GENOME_TAGS_STREAM_CHECKPOINT)
