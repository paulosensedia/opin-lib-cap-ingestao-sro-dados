...
from opin_lib_chassis_dados.movielens.streaming.ingestion.links_schema_ingestion_schema import LinksSchemaIngestion
...
links_topic = kafka_connection \
    .option("subscribe", "MOVIELENS_LINKS") \
...
schema = LinksSchemaIngestion.schema()
df_links = DataFrameFunctions.parse_json(links_topic, "value", "json_data", schema).select("json_data.*")


# COMMAND ----------
# DBTITLE 1,Write to bronze layer
DeltaLakeFunctions.write_delta_stream(df_links, EnumFormatTypes.DELTA.value,
    context.STORAGE_BRONZE_LINKS_STREAM,
    context.STORAGE_BRONZE_LINKS_STREAM_CHECKPOINT)
